...
export class Database {
  constructor() {
    ...
    this.supabaseReadUrl = 'https://clmquoqxsfesbjpseuhj.supabase.co/rest/v1/scores_view'; 
  ...
  async getAllScores() {
    try {
      const response = await fetch(`${this.supabaseReadUrl}?select=*`, {
        headers: {
          'apikey': this.supabaseKey,
          'Authorization': `Bearer ${this.supabaseKey}`,
        }
  ...
  async getTopScores(limit = 10) {
    try {
      const response = await fetch(`${this.supabaseReadUrl}?select=*&order=score.desc&limit=${limit}`, {
        headers: {
          'apikey': this.supabaseKey,
          'Authorization': `Bearer ${this.supabaseKey}`,
        }
